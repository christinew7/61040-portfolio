...
- I would add to the purpose that the owner has the option to not see purchases. The principle will be modified so the recipient can choose if they want to be surprised and not see purchases when they create a registry. The Registry state will have a `surprise` Flag with additional actions of <br>
  &nbsp;&nbsp;&nbsp;&nbsp;enableSurprise (registry: Registry) <br>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** registry exists<br>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** sets the `surprise` flag to true<br>
  &nbsp;&nbsp;&nbsp;&nbsp;disableSurprise (registry: Registry) <br>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** registry exists<br>
  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** sets the `surprise` flag to false<br>

7. Generic types

- This is preferable because the descriptions and prices can be automatically updated in real time, and the user doesn't have to manually update descriptions and prices when they change.
...
3. All usernames in the Users set is unique. It is preserved with the `register` action, which requires for the username to not already be associated with a User.
4. **state** <br>
   ...
   **actions** <br>
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this username doesn't exist in Users, this username doesn't exist in RegistrationRequests<br>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** generates a secret Token to be emailed to the user and saves this username, this password, and this token to RegistrationRequests<br>
   &nbsp;&nbsp;&nbsp;&nbsp;confirm (username: String, token: String): (user: User) <br>
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this username exists in RegistrationRequests, this token matches the token in RegistrationRequests<br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** creates a Token with this user and a randomized, unique token<br>
&nbsp;&nbsp;&nbsp;&nbsp;delete (user: User, token: String): <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this user is logged in and this token is associated with this user<br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** removes this Token associated with this user and this token<br>
...
PersonalAccessToken differs from PasswordAuthentication because a User can have multiple tokens, each of which can have limitations to what repositories can be accessed and expirations. I would add to the GitHub documentation that the access tokens are used to sign into their account securely and it should never be shared with anyone else. I would not use the word "password" because it diverges from the concept's purpose and is a little misleading.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** creates a ShortenedURL with the provider domain, followed by this alias if provided or a randomly generated unique alias if not provided<br>
...
**concept** BillableHours[Employee, Project] <br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this task isn't already associated with a Session with this project and this employee, this employee doesn't already have an ongoing Session <br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** adds the current date/time under endTime to the Session with this employee, this project, and this task <br>
&nbsp;&nbsp;&nbsp;&nbsp;endSessionManual (employee: Employee, project: Project, task: String, endTime: DateTime): <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this employee, this project, and this task are all associated in one Session, this Session has not ended yet, this endTime is after than this Session's startTime <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** adds this endTime to the Session with this employee, this project, and this task <br>
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** this room exists, there is no Booking associated with this Room <br>
...
&nbsp;&nbsp;&nbsp;&nbsp;editRoom (user: User, room: Room, startTime: Optional\<DateTime\>, endTime: Optional\<DateTime\>, newManagers: Optional\<Set\<Users\>\>, notes: Optional\<String\>): <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**requires** there is a Booking associated with this roomName, this user is in this Booking's set of managers, this endTime ends on or after this room's startTime if provided and are within the building's operating hours, this room doesn't already have a Booking where this startTime and this endTime overlap <br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;**effects** updates this Booking with the new fields this startTime, this endTime, this managers, and this notes, if applicable <br>
